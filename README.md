...
Objetivo:  
Desarrollar un asistente para guiar a los usuarios en la seleccion de productos y el posterior proceso de checkout (efectuar pago y generar el envio del producto) de manera conversacional. (Trabajo Practico N2)

Esquema:  
Usuario -> LLM -> Tool (busqueda de productos en ElasticSearch) -> Finalizacion de Compra y Envio

Tecnologias principales:  
Se utilizo un LLM de OpenAI como motor de IA del agente y se le asocio una Tool para realizar busquedas de productos en una base de datos ElasticSearch corriendo en un contenedor docker.

Interfase Grafica:  
Linea de comandos

Observaciones:  
Este ejemplo se desarollo para entender el funcionamiento del LLM. Unicamente se uso un system prompt para establecer el comportamiento esperado. El Agente se encarga de todo el proceso desde busqueda de productos hasta el envio final. 
...
Esquema:  
Usuario -> Agente Compras -> Definicion del carrito de compras -> Handoff a Checkout  
...
Objetivo:  
Replicar misma funcionalidad e implementacion del caso 2 pero exponiendo la solucion a traves de un servidor API desarrollado en express para posterior despliegue en produccion. (Plataforma Railway)


Esquema:  
Usuario -> Agente Compras -> Gestion del carrito de compras -> Handoff a Checkout  

Usuario -> Agente Checkout -> Realizacion de Pago y envio del producto


Tecnologias principales:  
Se utilizo Gemini 2.5 como motor de IA de ambos agentes y se se puso a disposicion un MCP Server para que los agentes autodecten todas las herramientas para llevar a cabo la gestion. Se desplego una base de datos ChromaDB coriendo en un contenedor docker. Se desplego un MCP Server desarrollado en node Express() y corriendo en un contenedor docker. El programa principal el cual es consumido por el usuario es un Servidor API desarrollado en node express desplegado como  servicio en Railway.
